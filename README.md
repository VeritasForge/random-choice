...
그 공백이 잘려 나가 서버가 가진 값과 영영 어긋납니다. 그러면 모든 조회가 401이 되는데
화면에는 원인이 보이지 않습니다.
...
**그래서 지금 미리보기에서는 조회가 되지 않습니다.** 실제로 미리보기를 하나 올려 재
보니 조회가 **본문 없는 HTTP 500**으로 끝납니다. 미리보기에는 `API_ORIGIN`이 없어
화면 서버가 `http://localhost:8080`으로 요청을 보내고, Vercel 함수 안에는 거기 듣는
것이 없어 중계가 예외로 끝나기 때문입니다. 화면은 그 500을 서버가 준 오류 코드로
읽지 못하므로 안내 문구도 고르지 못합니다.
