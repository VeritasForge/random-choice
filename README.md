...
**음식 종류를 두 단계로 자르는 규칙은 아직 실제 데이터로 확인되지 않았습니다.** `음식점 > 한식 > 육류,고기 > 곱창,막창`을 `한식 > 육류,고기`로 만드는 규칙인데, 근거로 삼은 실제 카카오 값이 하나뿐입니다. **열쇠를 받으면 가장 먼저 이것부터 확인하세요** — 같은 상위 분류 아래에 2단계짜리(`한식`)와 3단계 이상짜리(`한식 > 육류,고기`)가 함께 나오면, 사용자가 `한식`을 골랐을 때 고깃집들이 다른 카드 뒤에 숨습니다. 확인 방법과 판단 기준은 `docs/superpowers/plans/2026-08-29-lunch-random-picker.md`의 "남은 확인" 절에 있습니다.
