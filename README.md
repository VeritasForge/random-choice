...
├── internal/
    ...
    ├── kakao/              카카오와 이야기하는 유일한 창구. 중심과 두 링(400m·800m)을 회전시키며 조회해 합치고 거리를 다시 계산한다
...
**아직 없는 것들**: 지도 표시, 여러 명이 함께 고르기, 로그인, 주소를 직접 입력해 위치 지정, 점심 외의 상황(저녁·카페). 요청 빈도 제한도 없어서, 조회 한 건이 이제 카카오 호출을 최대 123번(중심 1곳 × 3페이지 + 링 8곳 × 3페이지 × 회전 5번) 쓰는데도 공개 배포한다면 한 사람이 하루 호출 한도(10만 건, 최대치 기준 하루 약 813명분)를 다 쓸 수 있습니다.
