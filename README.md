...
**서버를 먼저, 화면을 나중에 올리세요.** 화면은 `API_ORIGIN`을 빌드할 때 굳혀 넣기
때문에, 서버 주소가 바뀌면 화면을 다시 빌드해야 그 값이 반영됩니다.
...
**둘 다 실서비스(Production)에만 넣었습니다.** 미리보기 배포는 별도 자원을 받지 않아,
미리보기에도 열쇠를 넣으면 시험할 때마다 실서비스의 카카오 하루 한도를 함께 깎습니다.
지금 상태에서 미리보기를 열면 조회가 `not_configured`로 답합니다 — 고장이 아니라
그렇게 정해 둔 것입니다.
