...
const (
	...
	//
	// 열쇠를 받으면 먼저 확인할 것: 카카오 원본 문자열이 "음식점 > 한식"인 가게와
	// "음식점 > 한식 > 육류,고기"인 가게가 같은 상위 분류 아래 섞여 나오는가.
	// 섞여 나오면 각각 "한식"과 "한식 > 육류,고기"라는 서로 다른 카드로 갈라져,
	// 사용자가 "한식"을 골랐을 때 고깃집이 다른 카드 뒤에 숨는다.
	// 그때는 이 값을 다시 판단해야 한다.
	//
